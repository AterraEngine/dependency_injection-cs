...
// Code
// ---------------------------------------------------------------------------------------------------------------------
public class TieredServiceProvider(TieredServiceContainer serviceContainer) : ITieredServiceProvider {
    protected static readonly FrozenDictionary<Type, Func<TieredServiceProvider, object>> SpecialTypeResolvers = new Dictionary<Type, Func<TieredServiceProvider, object>> {
        { typeof(ITieredServiceProvider), static provider => provider },
        { typeof(ITieredServiceContainer), static provider => provider.ServiceContainer }
    }.ToFrozenDictionary();

    internal protected TieredServiceProvider? ParentScope { get; protected set; }
    protected int ServiceDepth { get; init; }

    internal ConcurrentDictionary<Guid, object> Instances { get; } = new();
    internal ConcurrentBag<ITieredServiceProvider> ChildScopes { get; } = [];

    protected TieredServiceContainer ServiceContainer { get; } = serviceContainer;
    ...
    private static readonly Lazy<MethodInfo> GetServiceMethod = new(static () => typeof(TieredServiceProvider)
    ...
    public virtual TService? GetService<TService>() where TService : class {
        ...
        if (SpecialTypeResolvers.TryGetValue(typeof(TService), out Func<TieredServiceProvider, object>? func)) return (TService)func(this);
        return null;
    }
    ...
    protected virtual TService ResolveServiceByScope<TService>(FrozenServiceRecord record) where TService : class
        => record.Depth switch {
            FrozenServiceRecord.KnownServiceDepth.ProviderScoped => ResolveProviderScoped<TService>(record),
            FrozenServiceRecord.KnownServiceDepth.Singleton => ServiceContainer.GetSingletonService<TService>(record.Id),
            FrozenServiceRecord.KnownServiceDepth.Transient => ServiceContainer.GetTransientService<TService>(record.Id),
            FrozenServiceRecord.KnownServiceDepth.CustomTier => ResolveCustomScoped<TService>(record),
    ...
    private TService ResolveCustomScoped<TService>(FrozenServiceRecord record) where TService : class {
        TieredServiceProvider? currentScope = this;

        while (currentScope != null) {
            if (record.ServiceDepth == currentScope.ServiceDepth) return currentScope.ResolveProviderScoped<TService>(record);
            if (record.ServiceDepth > currentScope.ServiceDepth) break;
    ...
    public void Dispose() {
        try {
            ...
            while (ChildScopes.TryTake(out ITieredServiceProvider? childScope)) {
                childScope.Dispose();
            }
        }
        finally {
    ...
    private void RemoveItemFromBag(TieredServiceProvider child) {
        if (ChildScopes.IsEmpty) return;

        var itemsToKeep = new List<ITieredServiceProvider>(ChildScopes.Count - 1);

        // Remove specific item, keeping others in memory
        while (ChildScopes.TryTake(out ITieredServiceProvider? item)) {
        ...
        foreach (ITieredServiceProvider item in itemsToKeep) {
